...
### 📌 明日接續任務（優先）(2026/02/13)
- **P0｜修復 0 筆品牌問題**：優先處理「九五之丹、寶齡富錦、大醫生技」為 0 筆的根因（sitemap 可達性、產品頁判斷規則、網域選擇器差異）。
- **P0｜補齊可追溯 Error Log**：在 `batch_scanner.py` / `agent_d2c_scanner.py` 增加結構化錯誤日誌（品牌、URL、錯誤類型、重試次數、最終狀態），並輸出到 `data/error_logs/`。
- **P1｜價格異常清理**：針對 `price=0/缺失` 的 3 筆資料做來源追查與修正（DOM selector fallback + 正則備援 + LLM 輔助抽取）。
- **P1｜品牌命名正規化**：統一 `vitabox / VITABOX / VITABOX®`，避免統計分裂與後續分析偏差。
- **P2｜驗收腳本強化**：更新 `validate_results.py`，新增「0 筆品牌告警、價格異常告警、錯誤日誌完整性檢查」。
